...
var hexToLimitedRange = function hexToLimitedRange(input, obj) {
  var slope = (obj.out_end - obj.out_start) / (obj.in_end - obj.in_start);
...
var app = new Vue({
  ...
  methods: {
    style: function style(val) {
      //位置と角度をハッシュから
      var tx = val.tx;
      var x = parseInt(tx.substr(1, 2), 16); //

      var y = parseInt(tx.substr(3, 2), 16);
      var r = parseInt(tx.substr(5, 2), 16);
      var top = hexToLimitedRange(x, widthSetting);
